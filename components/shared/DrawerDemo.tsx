...
// const [lat_, setlat] = useState(latitude ? latitude:"");
//const [lng_, setlng] = useState(longitude ? longitude:"");
  ...
  return (
    <Drawer open={isOpen} onOpenChange={setIsOpen}>
      <DrawerTrigger asChild>
        <Button onClick={() => setIsOpen(true)} variant="outline">{(!latitude || !longitude) ? (<div className="absolute bottom-10 left-4 z-5 bg-blue-600 text-white py-3 px-1  flex gap-1 items-center hover:bg-bule-700 rounded-sm">➕ Add Property Location</div>):(<div className="absolute bottom-10 left-4 z-5 text-sm text-white bg-black py-3 px-1 flex gap-1 items-center hover:bg-green-700 rounded-sm">📍 Property: ({latitude},{longitude})</div>)} </Button>
      </DrawerTrigger>
      <DrawerContent>
