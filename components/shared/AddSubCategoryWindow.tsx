...
const AddSubCategoryWindow: React.FC<AddSubCategoryWindowProps> = ({
  isOpen,
  onClose,
  onSaved,
}) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
      <div className="flex h-[90vh] w-full max-w-5xl flex-col rounded-[28px] border border-slate-200 bg-white p-4 shadow-2xl dark:bg-[#131B1E] dark:text-gray-300">
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          <CreateSubCategoryForm onSaved={onSaved} />
        </div>
      </div>
    </div>
  );
